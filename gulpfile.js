...
const PathSrc = './resources/assets/',
	...
	batObj = {
		uglifyJs: (jsSrc, jsOut) => [
	        ...
	        sourcemaps.write(DefPath.jsMapPath),
	        gulp.dest(jsOut)
		],
		minifyCss: (cssSrc, cssOut) => [
	        ...
	        sourcemaps.write(DefPath.cssMapPath),
	        gulp.dest(cssOut)
	    ],
...
gulp.task('delMapFile', function() {
	gutil.log( gutil.colors.red('开始清除map文件，重新生成没有映射的js与css') );
...
if( renewVersion ){
	defTask = defTask.push('renewVersion');//更新env下的时间戳
}
...
if( production ){ //如果是线上环境先移除掉已生成的js和css，不生成map映射
	...
	gulp.task('default', function(callback) {
		runSequence('delMapFile', //清除原先的压缩文件及map映射
					 defTask);
	});
} else {
